#!/usr/bin/env python3
"""
GPQA / ARC (and other) benchmarks on a cloud GPU via `Modal <https://modal.com>`_.
...
default is ``B200:1`` to preserve the prior behavior.

Modal parses its own CLI first. Pass benchmark flags in one of these ways:

1. **Double dash** (everything after ``--`` goes to ``src.run_benchmark``)::

       modal run run_benchmark_b200.py --gpu H100:1 -- \
           --dataset gpqa_diamond --batch-size 128 --model Qwen/Qwen2.5-14B-Instruct --mode layered

2. **Modal entrypoint arg** (single string; avoids Modal "unexpected extra arguments")::

       modal run run_benchmark_b200.py \
           --gpu H100:1 \
           --benchmark-args "--dataset gpqa_diamond --batch-size 128 --model Qwen/Qwen2.5-14B-Instruct --mode layered"

3. **Environment** (no extra CLI tokens after the script path)::

       export LAYERSTREAM_GPU=H100:1
       export LAYERSTREAM_BENCHMARK_ARGS="--dataset gpqa_diamond --batch-size 128 --model Qwen/Qwen2.5-14B-Instruct --mode layered"
       modal run run_benchmark_b200.py
...
it is forwarded to the worker as an ephemeral Modal Secret so gated models / datasets
(GPQA, Llama, etc.) work out of the box. You can still reference named Modal Secrets
by exporting ``MODAL_SECRET_NAMES=huggingface,...``.
"""
...
_APP_NAME = "layerstream-benchmark"
...
def _benchmark_argv_from_modal_cli() -> list[str]:
...
def run_benchmark_remote(benchmark_argv: list[str]) -> int:
    """Run ``python -m src.run_benchmark`` on a GPU worker (GPU type set via ``with_options``)."""
    import sys as _sys

    root = Path(_REMOTE_ROOT)
    if not (root / "src" / "run_benchmark.py").is_file():
        print(f"ERROR: expected LayerStream at {root}", file=_sys.stderr)
        return 2
    return int(
        subprocess.call(
            [_sys.executable, "-m", "src.run_benchmark", *benchmark_argv],
            cwd=str(root),
        )
    )


def _resolve_benchmark_argv(benchmark_args: str) -> list[str]:
    """Priority: explicit Modal param -> env -> ``--`` passthrough."""
    s = benchmark_args.strip()
    if s:
        return shlex.split(s)
    env = os.environ.get("LAYERSTREAM_BENCHMARK_ARGS", "").strip()
    if env:
        return shlex.split(env)
    return _benchmark_argv_from_modal_cli()


@app.local_entrypoint()
def main(gpu: str = "", benchmark_args: str = "") -> None:
    ...
    benchmark_argv = _resolve_benchmark_argv(benchmark_args)
    if not benchmark_argv:
        print(
            "No arguments for src.run_benchmark. Use one of:\n\n"
            "  modal run run_benchmark_b200.py --gpu H100:1 -- --dataset gpqa_diamond "
            "--batch-size 128 --model Qwen/Qwen2.5-14B-Instruct --mode layered\n\n"
            "  modal run run_benchmark_b200.py --gpu H100:1 --benchmark-args "
            '"--dataset gpqa_diamond --batch-size 128 --model Qwen/Qwen2.5-14B-Instruct --mode layered"\n\n'
            "  export LAYERSTREAM_GPU=H100:1\n"
            "  export LAYERSTREAM_BENCHMARK_ARGS='--dataset gpqa_diamond ...'\n"
            "  modal run run_benchmark_b200.py\n",
            file=sys.stderr,
        )
        raise SystemExit(2)
    print(f"[layerstream] launching on gpu={_GPU_SPEC}", file=sys.stderr)
    code = int(run_benchmark_remote.remote(benchmark_argv))
