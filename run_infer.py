...
def _build_vram_engine(
    ...
    """Load every decoder layer directly to GPU — no streaming."""
    print(f"[vram] Moving {len(parts.layers)} layers to GPU …")
    vram_layers = [
        layer.to(device=cfg.model.device, dtype=torch_dtype).eval()
...
def main() -> None:
    ...
    print(f"Loading model: {cfg.model.model_name_or_path}  (dtype={cfg.model.dtype})")
    logger = ExperimentLogger(cfg.logging.log_dir)
    parts = load_model_parts(cfg.model.model_name_or_path, dtype=cfg.model.dtype)
    torch_dtype = getattr(torch, cfg.model.dtype)

    if cfg.offload.vram_only:
        engine, kv_manager = _build_vram_engine(parts, cfg, logger, torch_dtype)
    else:
